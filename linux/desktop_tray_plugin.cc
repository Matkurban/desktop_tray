...
//
//   2.  app_indicator_set_menu() is called ONLY ONCE, and only AFTER the
//       menu has been populated for the first time.
//
...
static bool g_menu_bound       = false;
...
static FlMethodResponse* handle_set_context_menu(FlValue* args) {
  ...
  // 3. Bind menu → indicator exactly once (now that it has children).
  if (g_indicator != nullptr && !g_menu_bound) {
