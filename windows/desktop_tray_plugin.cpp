...
class DesktopTrayPlugin : public flutter::Plugin {
 ...
 private:
  ...
  void Destroy(std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
  void SetIcon(const flutter::EncodableMap& args,
               std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
  void SetToolTip(const flutter::EncodableMap& args,
                  std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
  void SetContextMenu(const flutter::EncodableMap& args,
                      std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
...
void DesktopTrayPlugin::BuildMenu(HMENU menu,
  ...
  for (const auto& item_value : items) {
    ...
    auto type = std::get<std::string>(item_map.at(flutter::EncodableValue("type")));
    auto label = std::get<std::string>(item_map.at(flutter::EncodableValue("label")));
    ...
    if (type == "separator") {
      ...
      if (type == "checkbox") {
        ...
        BuildMenu(sub_menu, std::get<flutter::EncodableMap>(
            item_map.at(flutter::EncodableValue("submenu"))));
        item_id = reinterpret_cast<UINT_PTR>(sub_menu);
      }
      AppendMenuW(menu, uFlags, item_id,
                  converter_.from_bytes(label).c_str());
    }
  }
}

std::optional<LRESULT> DesktopTrayPlugin::HandleWindowProc(
...
void DesktopTrayPlugin::SetIcon(
    const flutter::EncodableMap& args,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  auto iconPath = std::get<std::string>(
      args.at(flutter::EncodableValue("iconPath")));
  ...
  nid_.hIcon = static_cast<HICON>(
      LoadImage(nullptr, converter_.from_bytes(iconPath).c_str(),
...
void DesktopTrayPlugin::SetToolTip(
  ...
  auto toolTip = std::get<std::string>(
      args.at(flutter::EncodableValue("toolTip")));
